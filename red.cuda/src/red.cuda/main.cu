...
void print_info(ostream& sout, const pp_disk* ppd, integrator *intgr, ttt_t dt, clock_t* sum_time_of_steps, clock_t* time_of_one_step, time_t* time_info_start)
{
	...
	*time_info_start = clock();
	...
	cout << ", Nc: "  << setw(5) << ppd->n_collision[	   EVENT_COUNTER_NAME_TOTAL]
		 << ", Nce: " << setw(5) << ppd->n_close_encounter[EVENT_COUNTER_NAME_TOTAL]
...
int main(int argc, const char** argv, const char** env)
{
	...
	try
	{
		...
		while (ppd->t <= opt.param->stop_time)
		{
			...
			if (!opt.param->inner_steps && (0.0 < opt.param->thrshld[THRESHOLD_RADII_ENHANCE_FACTOR]) && ppd->check_for_close_encounter(opt.param->inner_steps))
			{
				//inner_steps false
				ppd->print_event_data(*event_f, *log_f);
				ppd->clear_event_counter();
			...
			if (opt.param->close_encounter && opt.param->inner_steps)
			{
				//inner_steps true & close_encounter true
				if (0.0 < opt.param->thrshld[THRESHOLD_RADII_ENHANCE_FACTOR] && ppd->check_for_close_encounter(opt.param->inner_steps))
				{
					ppd->print_event_data(*event_f, *log_f);
					ppd->clear_event_counter();
					if (opt.param->output_type == OUTPUT_TYPE_TEXT)
