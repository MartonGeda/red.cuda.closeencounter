...
for i=1:2
   if i == 1
    ...
    p(2) = 0.940;
    set(lh,'Position',p);
    text(1.495978707e8 / 86400 * phase1(ii(1),4),phase1(ii(1),1),'   \leftarrow');
    text(1.495978707e8 / 86400 * phase2(ii(1),4),phase2(ii(1),1),'\rightarrow   ','HorizontalAlignment','right');        
    p = get(gca,'position');
    p(1) = p(1) - 0.03;
    set(gca,'position',p);         

    subplot(3,3,2);
    plot(1.495978707e8 / 86400 * phase1(ii,5),phase1(ii,2),'*',1.495978707e8 / 86400 * phase2(ii,5),phase2(ii,2),'*');
    title('y - v_y');
    xlabel('v_y (km/s)');
    ylabel('y (AU)');          
    text(1.495978707e8 / 86400 * phase1(ii(1),5),phase1(ii(1),2),'   \leftarrow');
    text(1.495978707e8 / 86400 * phase2(ii(1),5),phase2(ii(1),2),'\rightarrow   ','HorizontalAlignment','right');
    p = get(gca,'position');
    p(1) = p(1) - 0.03;
    set(gca,'position',p);         

    subplot(3,3,3);
    [hAx,hLine1,hLine2] = plotyy(t(ii) - t(ii(1)),dv(ii),t(ii) - t(ii(1)),fij(ii)./fsun(ii,1));
    ...
    subplot(3,3,6);
    plot(oe1(ii,6),oe1(ii,1),'*',oe2(ii,6),oe2(ii,1),'*');
    title('a - M');
    xlabel('M (rad)');
    ylabel('a (AU)'); 
    text(oe1(ii(1),6),oe1(ii(1),1),'   \leftarrow');
    text(oe2(ii(1),6),oe2(ii(1),1),'\rightarrow   ','HorizontalAlignment','right');        
    p = get(gca,'position');
    p(1) = p(1) - 0.03;
    set(gca,'position',p);          

    subplot(3,3,7);
    plot(oe1(ii,4),oe1(ii,2),'*',oe2(ii,4),oe2(ii,2),'*');
    title('e - w');
    xlabel('w (rad)');
    ylabel('e (-)'); 
    text(oe1(ii(1),4),oe1(ii(1),2),'   \leftarrow');
    text(oe2(ii(1),4),oe2(ii(1),2),'\rightarrow   ','HorizontalAlignment','right');        
    p = get(gca,'position');
    p(1) = p(1) - 0.03;
    set(gca,'position',p);  
